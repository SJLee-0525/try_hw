...
def create_user(receive_data_list):
    ...
    for num, pre_result in enumerate(result):
        if pre_result == True:
            fin_results.append(receive_data_list[num])
        elif pre_result == 'blocked':
            count += 1
        elif tuple(pre_result)[0] == False:
            for dict_data_index in range(1, len(pre_result)):
                receive_data_list[num][pre_result[dict_data_index]] = None
            count += 1
            fin_results.append(receive_data_list[num])
    
    print(f'잘못된 데이터로 구성된 유저의 수는 {count} 입니다.')
    for fin_result in fin_results:
        pprint(fin_result) # 딕셔너리 이쁘게 출력해주는 함수


def is_validation(receive_data_dict):
    # 잘못된 부분들을 정리해서 list 반환
    check = True
    temp_list = []
    if receive_data_dict['company'] in black_list:
        temp_list.append('company')
        return 'blocked'  
     
    else:
        if receive_data_dict['blood_group'] not in blood_types:
            temp_list.append('blood_group')
            check = False
        if '@' not in list(receive_data_dict['mail']):
            temp_list.append('mail')
            check = False
        if len(receive_data_dict['name']) < 2 or len(receive_data_dict['name']) > 30:
            temp_list.append('name')
            check = False
        if len(receive_data_dict['website']) < 1:
    ...
    if check == True:
        ...
        if len(temp_list) == 1:
            return(False, temp_list[0])
        else:
            return(False, temp_list)

    
result_list = create_user(user_data)